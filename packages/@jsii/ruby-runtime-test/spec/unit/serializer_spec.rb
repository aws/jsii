...
RSpec.describe Jsii::Serializer do
  describe '.dump' do
    ...
    it 'raises TypeError for plain Ruby objects (does NOT walk or stringify host objects)' do
      # The previous monkey-patched Object#jsii_serialize would have walked any
      # object with an @jsii_ref ivar.  Untyped host objects must not be walked
      # — and must not silently pass through either, because JSON.generate
      # would stringify them into "#<Object:0x...>" garbage on the wire.  They
      # fail fast with an error naming the class instead.
      host_object = Object.new
      host_object.instance_variable_set(:@jsii_ref, 'this is a host detail, not jsii state')

      expect { described_class.dump(host_object) }
        .to raise_error(TypeError, /cannot serialize instance of Object to the jsii wire format/)
    end
  end
