...
RSpec.describe 'JSII compliance: interfaces' do
  ...
  # Ruby-side ergonomics on top of the compliance surface above (deliberately
  # NOT a suite-tagged case — the shared suite is upstream's to extend): the
  # generated bindings coerce a bare closure into a single-abstract-method
  # interface implementation at the call site, end-to-end through the kernel.
  it 'coerces closures into single-method interfaces' do
    consumer = JsiiCalc::ConsumerCanRingBell.new
