...
export default class JavaPackageMaker extends Target {
    ...
    public async build(sourceDir: string, outDir: string): Promise<void> {
        ...
        const userXml = await this.generateMavenSettingsForLocalDeps(outDir);
        try {
            await this.runCommand('mvn', [...mvnArguments, 'package', '-D', `publish.url=${url}`, `--settings=${userXml}`], { cwd: sourceDir });
        } finally {
            await fs.remove(userXml);
        }
    }

    private async generateMavenSettingsForLocalDeps(currentOutputDirectory: string) {
        const tmpdir = await fs.mkdtemp('/tmp/pacmak-java-maven-settings');
        const filePath = path.join(tmpdir, 'user.xml');
        ...
        <settings xmlns="http://maven.apache.org/POM/4.0.0"
            ...
            <activeProfiles>
                <activeProfile>local-jsii-modules</activeProfile>
            </activeProfiles>
        </settings>
        `);
        return filePath;
