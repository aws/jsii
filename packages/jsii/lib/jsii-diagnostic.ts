...
export class JsiiDiagnostic implements ts.Diagnostic {
  ...
  public addRelatedInformation(diag: JsiiDiagnostic): this {
    this.relatedInformation.push(diag);
    return this;
  }
}

// eslint-disable-next-line @typescript-eslint/no-namespace
export namespace JsiiDiagnostic {
  ...
  > {
    ...
    public static readonly JSII_0004_COULD_NOT_FIND_ENTRYPOINT = Code.error({
      code: 4,
      formatter: (mainFile: string) =>
        `Could not find "main" file: ${mainFile}`,
    ...
    public static readonly JSII_1002_UNSPECIFIED_PROMISE = Code.error({
      code: 1002,
      formatter: () =>
        `Un-specified promise type. Specify it using "Promise<T>"`,
    ...
    public static readonly JSII_3004_INVALID_SUPERTYPE = Code.error({
      ...
      formatter: (
        ...
        badDeclaration: ts.Declaration,
      ) => {
        return `Illegal ${clauseType(
          clause.token,
        )} clause for an exported API: ${ts.SyntaxKind[badDeclaration.kind]}`;
    ...
    public static readonly JSII_9999_RELATED_INFO = Code.suggestion({
      code: 9999,
      formatter: (str: string) => str,
    ...
    //////////////////////////////////////////////////////////////////////////////
    private static message<T extends DiagnosticMessageFormatter>({
      code,
      name,
      formatter,
    }: {
    ...
    private static suggestion<T extends DiagnosticMessageFormatter>({
      code,
      name,
      formatter,
    }: {
    ...
    private static warning<T extends DiagnosticMessageFormatter>({
      code,
      name,
      formatter,
    }: {
    ...
    private static error<T extends DiagnosticMessageFormatter>({
      code,
      name,
      formatter,
    }: {
